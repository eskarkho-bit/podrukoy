...
/**
 * Фото в переписке заявки. Имя уникальное, а не фиксированное: переписка —
 * журнал, повторная отправка — новое сообщение, а не замена старого снимка.
 * Путь лежит под заявкой, чтобы удаление аккаунта стёрло его общим префиксом,
 * но в отдельной папке chat/ — у неё свои правила чтения: только стороны
 * сделки и модератор.
 */
export function uploadChatPhoto(
  orderId: string,
  senderUid: string,
  localUri: string,
): Promise<string> {
  return upload(`orders/${orderId}/chat/${senderUid}-${Date.now()}.jpg`, localUri);
}
